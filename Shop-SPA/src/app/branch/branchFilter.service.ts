...
import { Branch, SearchResult } from './branch';
...
*/
function sort(orders: Branch[], column: string, direction: string): Branch[] {
  if (direction === '') {
    return orders;
  } else {
...
*/
function matches(orders: Branch, term: string, pipe: PipeTransform) {
  return orders.name.toLowerCase().includes(term);
}
...
export class BranchFilterService {
  ...
  // tslint:disable-next-line: variable-name
  private _orders$ = new BehaviorSubject<Branch[]>([]);
  // tslint:disable-next-line: variable-name
  ...
  constructor(private pipe: DecimalPipe, private branchService: BranchService) {
