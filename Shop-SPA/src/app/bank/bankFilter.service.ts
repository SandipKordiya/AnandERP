...
import { Bank, SearchResult } from './bank';
...
*/
function sort(orders: Bank[], column: string, direction: string): Bank[] {
  if (direction === '') {
    return orders;
  } else {
...
*/
function matches(orders: Bank, term: string, pipe: PipeTransform) {
  return orders.bankName.toLowerCase().includes(term);
}
...
// tslint:disable-next-line: variable-name
private _orders$ = new BehaviorSubject<Bank[]>([]);
// tslint:disable-next-line: variable-name
...
constructor(private pipe: DecimalPipe, private bankService: BankService) {
