...
import { Brand, SearchResult } from '../_models/brand';
...
*/
function sort(orders: Brand[], column: string, direction: string): Brand[] {
  if (direction === '') {
    return orders;
  } else {
...
*/
function matches(orders: Brand, term: string, pipe: PipeTransform) {
...
export class BrandFilterService {
  ...
  // tslint:disable-next-line: variable-name
  private _orders$ = new BehaviorSubject<Brand[]>([]);
  // tslint:disable-next-line: variable-name
  ...
  constructor(private pipe: DecimalPipe, private brandService: BrandService) {
