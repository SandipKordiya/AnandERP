...
export class StockReturnComponent implements OnInit {
  ...
  // public  isForStock = new BehaviorSubject(false);
  ...
  constructor(private http: HttpClient, private branchService: BranchService,
    ...
    this.breadCrumbItems = [{ label: 'Stock Return' }, { label: 'New Stock Return', active: true }];
  ...
  //   if(value) this.model.isForStock = false;
  //   if(!value) this.model.isForStock = true;
  //   console.log('this.model.isForStock', this.model.isForStock)
  // }
  ...
  submitPurchase() {
    const SaleModel = {
      ...
      description: null,
      stockReturnItems: this.products
    };
    console.log('Purcahse Return', SaleModel);
    this.shopService.addStockReturn(this.currentUser, SaleModel).subscribe(next => {
