...
import { Salereturn, SaleReturnSearchResult } from '../sale-return-list/salereturn.model';
 ...
 */
function sort(orders: Salereturn[], column: string, direction: string): Salereturn[] {
    if (direction === '') {
        return orders;
    } else {
 ...
 */
function matches(orders: Salereturn, term: string, pipe: PipeTransform) {
    return orders.id.toString().includes(term)
        || orders.invoiceNo.toLowerCase().includes(term)
        || orders.saleReturnDate.toString().toLowerCase().includes(term)
...
export class SaleReturnOrderService {
    // tslint:disable-next-line: variable-name
    private _loading$ = new BehaviorSubject<boolean>(true);
    // tslint:disable-next-line: variable-name
    private _search$ = new Subject<void>();
    // tslint:disable-next-line: variable-name
    private _orders$ = new BehaviorSubject<Salereturn[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    constructor(private pipe: DecimalPipe, private shopService: ShopService) {
        ...
        this.shopService.getSellReturnList(branchId)
            .subscribe((res: any) => {
                console.log('sale return service', res);
     ...
     * Search Method
     */
    private _search(): Observable<SaleReturnSearchResult> {
