...
import { StockModel, StockSearchResult } from './stock-model';
 ...
 */
function sort(orders: StockModel[], column: string, direction: string): StockModel[] {
    if (direction === '') {
        return orders;
    } else {
 ...
 */
function matches(orders: StockModel, term: string, pipe: PipeTransform) {
...
export class StockOrderService {
    ...
    // tslint:disable-next-line: variable-name
    private _orders$ = new BehaviorSubject<StockModel[]>([]);
    // tslint:disable-next-line: variable-name
     ...
     * Search Method
     */
    private _search(): Observable<StockSearchResult> {
