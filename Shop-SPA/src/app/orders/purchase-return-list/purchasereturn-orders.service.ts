...
import { PurchaseReturnOrder, PurchaseReturnSearchResult } from './purchasereturn.model';
import { SortDirection } from './purchase-return-sortable.directive';
 ...
 */
function sort(orders: PurchaseReturnOrder[], column: string, direction: string): PurchaseReturnOrder[] {
    if (direction === '') {
        return orders;
    } else {
 ...
 */
function matches(orders: PurchaseReturnOrder, term: string, pipe: PipeTransform) {
    return orders.id.toString().includes(term)
        || orders.invoiceNo.toLowerCase().includes(term)
        || orders.purchaseReturnDate.toString().toLowerCase().includes(term)
...
export class PurchaseReturnOrderService {
    // tslint:disable-next-line: variable-name
    private _loading$ = new BehaviorSubject<boolean>(true);
    // tslint:disable-next-line: variable-name
    private _search$ = new Subject<void>();
    // tslint:disable-next-line: variable-name
    private _orders$ = new BehaviorSubject<PurchaseReturnOrder[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    constructor(private pipe: DecimalPipe, private shopService: ShopService) {
        this.shopService.getPurchaseReturnList()
            .subscribe((res: any) => {
                console.log('Purchase return service', res);
     ...
     * Search Method
     */
    private _search(): Observable<PurchaseReturnSearchResult> {
