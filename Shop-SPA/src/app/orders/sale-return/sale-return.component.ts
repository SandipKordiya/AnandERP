...
export class SaleReturnComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private branchService: BranchService,
    ...
    this.breadCrumbItems = [{ label: 'Sale Returns' }, { label: 'New Sale Return Invoice', active: true }];
  ...
  selectPartyEvent(item) {
    console.log(item);
    var data = this.states.find(x => x.id === item.state);
  ...
  submitPurchase() {
    const SaleModel = {
      ...
      description: null,
      saleReturnItems: this.products
    };
    console.log('SaleModel', SaleModel);
    this.shopService.addSaleReturn(this.currentUser, SaleModel).subscribe(next => {
  ...
  getProductsByProductId(id: number): void {
    this.shopService.getProductsSaleByProductIdAndPartyId(id, this.partyId).subscribe(next => {
