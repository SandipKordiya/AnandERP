...
export class SaleReturnComponent implements OnInit {
  ...
  constructor(
    ...
    this.breadCrumbItems = [
      { label: 'Sale Returns' },
      { label: 'New Sale Return Invoice', active: true },
    ];
  }

  // form input values
  addSaleReturnForm = new FormGroup({
  ...
  get partyData() {
    return this.addSaleReturnForm.get('party');
  }

  get productDataFeild() {
    return this.addSaleReturnForm.get('productData');
  }
  ...
  generateInvoiceNo() {
    ...
    this.addSaleReturnForm.patchValue({
  ...
  getProductsByProductId(id: number): void {
    this.shopService
      .getProductsSaleByProductIdAndPartyId(id, this.partyData.value.id)
      .subscribe(
        (next) => {
  ...
  public onChangeProduct(e) {
    ...
    this.getProductsByProductId(e.id);
  ...
  getSchemeByProductId() {
    ...
    let f = this.addSaleReturnForm.value;
  ...
  public onChangeBatchProduct() {
    let e = this.addSaleReturnForm.value.batchNumber;
  ...
  public ChangeTotalAmount() {
    ...
    // values from form
    let _quantity = this.addSaleReturnForm.value.quantity;
    let _rate = this.addSaleReturnForm.value.rate;
    let _discount = this.addSaleReturnForm.value.discount;
    let _otherDiscount = this.addSaleReturnForm.value.otherDiscount;
    let _taxid = this.addSaleReturnForm.value.Tax;
    let _mrp = this.addSaleReturnForm.value.mrp;
    // TaxPercentage
    // taxe rate
    try {
      let mrp = _mrp;
  ...
  private addNewProduct() {
    let formData = this.addSaleReturnForm.value;
    const item = {
  ...
  AddProduct() {
    ...
    this.emptyForm();
    console.log(this.addSaleReturnForm.value);
  }

  submitSaleReturn(finalComponent) {
    this.spinner.show();
    const SaleModel: any = {
      invoiceNo: this.MainPostObject.invoiceNo,
      partyId: this.MainPostObject.partyId,
      taxType: this.MainPostObject.TaxType,
      purchaseReturnDatesaleDate: moment(this.MainPostObject.date).format(
        'YYYY-M-D'
      ),
    ...
    if (this.orderId === undefined) {
      SaleModel.purchaseReturnItems = this.products;
      this.shopService.addSaleReturn(this.currentUser, SaleModel).subscribe(
        (next) => {
          this.alertify.success('New sale return entry Added.');
          this.router.navigate(['/order/sale/return/list']);
        },
        (error) => {
          this.alertify.error(error.error);
        }
