...
import { StockTransferModel, StockTransferSearchResult } from './stock-transfer-model';
 ...
 */
function sort(orders: StockTransferModel[], column: string, direction: string): StockTransferModel[] {
    if (direction === '') {
        return orders;
    } else {
 ...
 */
function matches(orders: StockTransferModel, term: string, pipe: PipeTransform) {
...
export class StockTransferOrderService {
    ...
    // tslint:disable-next-line: variable-name
    private _orders$ = new BehaviorSubject<StockTransferModel[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    constructor(private pipe: DecimalPipe, private shopService: ShopService) {
        ...
        this.shopService.getStockTransferList(branchId)
            .subscribe((res: any) => {
     ...
     * Search Method
     */
    private _search(): Observable<StockTransferSearchResult> {
