...
export class ProductService {
  ...
  viewProduct(id: number): Observable<Product> {
    return this.http.get<Product>(this.baseUrl + 'product/GetProductInfo/' + id);
  }
  ...
  upload(file: File): Observable<HttpEvent<any>> {
    ...
    const req = new HttpRequest('POST', `${this.baseUrl}product/ImportExcelFile`, formData, {
      reportProgress: true,
      responseType: 'json'
    });
    ...
    getScheme(productId: number): Observable<any> {
      return this.http.get<any>(this.baseUrl + 'productscheme/GetScheme/' + productId);
    }
