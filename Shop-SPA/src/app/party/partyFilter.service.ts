 ...
 */
function sort(orders: Party[], column: string, direction: string): Party[] {
    if (direction === '') {
        return orders;
    } else {
 ...
 */
function matches(orders: Party, term: string, pipe: PipeTransform) {
    return orders.id.toString().includes(term)
        || orders.name.toLowerCase().includes(term)
        || orders.invoiceName.toString().toLowerCase().includes(term)
        // || orders.mobile.toLowerCase().includes(term)
        || orders.email.toString().toLowerCase().includes(term)
        || orders.gstin.toLowerCase().includes(term);
        // || orders.pincode.toString().toLowerCase().includes(term);
}
...
export class PartyFilterService {
    ...
    // tslint:disable-next-line: variable-name
    private _orders$ = new BehaviorSubject<Party[]>([]);
    // tslint:disable-next-line: variable-name
    ...
    constructor(private pipe: DecimalPipe, private partyService: PartyService) {
